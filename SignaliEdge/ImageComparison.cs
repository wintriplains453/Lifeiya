...
namespace SignaliEdge
{
    class ImageComparison
    {
        ...
        public Mat FASTDetector(Image<Bgr, byte> img)
        {
            var gray = img.Convert<Gray, byte>();

            FastFeatureDetector detector = new FastFeatureDetector(10);
            var corners = detector.Detect(gray);

            Mat outing = new Mat();
            Features2DToolbox.DrawKeypoints(gray, new VectorOfKeyPoint(corners), outing, new Bgr(0, 255, 0));

            corners = corners.OrderBy(i => i.Point.Y).ThenBy(i => i.Point.X).ToArray();
        ...
        public Mat Draw(Mat modelImage, Mat observedImage, out long matchTime, out long score)
        {
            Mat homography;
            ...
            using (VectorOfVectorOfDMatch matches = new VectorOfVectorOfDMatch())
            {
                ...
                Features2DToolbox.DrawMatches(modelImage, modelKeyPoints, observedImage, observedKeyPoints,
                   matches, result, new MCvScalar(0, 255, 0), new MCvScalar(255, 255, 255), mask);
